...
dgm_combined <- function(n, rg, prs_beta_child, prs_beta_adult, cancer_prev,
                         ...
                         interaction_sel, return_probs = FALSE) {
  prs <- MASS::mvrnorm(n, mu = c(0, 0),
  ...
  if (return_probs) {
    out$selection_prob <- selection_prob
  }
...
# ------------------------------------------------------------------------------
# Summarize average selection probabilities across parameter values
# ------------------------------------------------------------------------------

summarize_selection_prob <- function(child_vals, cancer_vals, interaction_vals, n = 1e5) {
  grid <- expand.grid(
  ...
  results <- lapply(1:nrow(grid), function(i) {
    ...
    mean_prob <- mean(plogis(
      dat$prs_child * 0.1 * grid$bodysize_sel_child[i] +
      dat$prs_adult * 0.1 * 0 + # adult selection fixed at 0
      dat$cancer * grid$cancer_sel[i] +
      (dat$prs_child * 0.1 * dat$cancer) * grid$interaction_sel[i]
    ))
...
# Run summary and save as data frame
selection_summary <- summarize_selection_prob(
  child_vals = seq(-5, 0, by = 1),
  cancer_vals = seq(0, -5, by = -1),
  interaction_vals = c(0, -2.5, -5)
)
...
# Add selection probabilities for interpretation
sim_results <- left_join(sim_results, selection_summary,
